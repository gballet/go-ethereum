	...
	if gasLimitOverride {
		...
		if blockGasLimit > 0 {
			if header.GasLimit != blockGasLimit {
				panic(fmt.Sprintf("Block gas limit doesn't match BlockGasLimitContract with AuRa: %d != %d at block %d, merged=%d", header.GasLimit, blockGasLimit, header.Number, c.isPos))
			}
		}
	}
}
