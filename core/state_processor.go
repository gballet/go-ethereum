...
func VerkleTransition(statedb *state.StateDB) error {
	...
	if migrdb.InTransition() {
		...
		if err != nil {
			return fmt.Errorf("error getting account iterator: %w", err)
		}
		defer accIt.Release()
		accIt.Next()
		...
		for count < maxMovedCount {
			...
			if err != nil {
				...
				return fmt.Errorf("error deserializing account: %w", err)
			}
			...
			if acc.HasStorage() {
				...
				if err != nil {
					return fmt.Errorf("error getting storage iterator: %w", err)
				}
				stIt.Next()
				...
				for ; !migrdb.GetStorageProcessed() && count < maxMovedCount; count++ {
					...
					if err := rlp.DecodeBytes(stIt.Slot(), &value); err != nil {
						return fmt.Errorf("error decoding snapshot slot bytes %x: %w", stIt.Slot(), err)
					}
...
func (p *StateProcessor) Process(block *types.Block, statedb *state.StateDB, cfg vm.Config) (types.Receipts, []*types.Log, uint64, error) {
	...
	err := VerkleTransition(statedb)
	if err != nil {
		return nil, nil, 0, fmt.Errorf("error performing overlay transition: %w", err)
	}
