...
func goGetTxContext(handle C.uintptr_t) C.struct_evmc_tx_context {
	...
	if len(evm.TxContext.BlobHashes) > 0 {
		blobSlice := make([]C.evmc_bytes32, len(evm.TxContext.BlobHashes))
		for i, h := range evm.TxContext.BlobHashes {
			blobSlice[i] = evmcHash(h)
		}
		txCtx.blob_hashes = &blobSlice[0]
		txCtx.blob_hashes_count = C.size_t(len(blobSlice))
	}
