...
func opCodeCopy(pc *uint64, interpreter *EVMInterpreter, scope *ScopeContext) ([]byte, error) {
	...
	if interpreter.evm.chainRules.IsEIP4762 && !scope.Contract.IsDeployment {
		statelessGas, wanted := interpreter.evm.Accesses.TouchCodeChunksRangeAndChargeGas(codeAddr[:], copyOffset, nonPaddedCopyLength, uint64(len(scope.Contract.Code)), false, false, scope.Contract.Gas)
...
func opExtCodeCopy(pc *uint64, interpreter *EVMInterpreter, scope *ScopeContext) ([]byte, error) {
	...
	if interpreter.evm.chainRules.IsEIP4762 {
		...
		statelessGas, wanted := interpreter.evm.Accesses.TouchCodeChunksRangeAndChargeGas(addr[:], copyOffset, nonPaddedCopyLength, uint64(len(contract.Code)), false, false, scope.Contract.Gas)
...
func getBlockHashFromContract(number uint64, statedb StateDB, witness *state.AccessWitness, availableGas uint64) (common.Hash, uint64) {
	...
	statelessGas := witness.TouchSlotAndChargeGas(params.HistoryStorageAddress[:], pnum, false, false, availableGas, false)
...
func opJump(pc *uint64, interpreter *EVMInterpreter, scope *ScopeContext) ([]byte, error) {
	...
	if !scope.Contract.validJumpdest(&pos) {
		statelessGas, wanted := interpreter.evm.TxContext.Accesses.TouchCodeChunksRangeAndChargeGas(scope.Contract.CodeAddr[:], pos.Uint64(), 1, uint64(len(scope.Contract.Code)), false, false, scope.Contract.Gas)
		scope.Contract.UseGas(statelessGas)
		if statelessGas < wanted {
			return nil, ErrOutOfGas
		}
	...
	*pc += 1
	if *pc < codeLen {
		...
		if !scope.Contract.IsDeployment && interpreter.evm.chainRules.IsVerkle && *pc%31 == 0 {
			...
			codeAddr := scope.Contract.CodeAddr
			statelessGas, wanted := interpreter.evm.Accesses.TouchCodeChunksRangeAndChargeGas(codeAddr[:], *pc, uint64(1), uint64(len(scope.Contract.Code)), false, false, scope.Contract.Gas)
			scope.Contract.UseGas(statelessGas)
			if statelessGas < wanted {
				return nil, ErrOutOfGas
			}
...
func makePush(size uint64, pushByteSize int) executionFunc {
	return func(pc *uint64, interpreter *EVMInterpreter, scope *ScopeContext) ([]byte, error) {
		...
		if !scope.Contract.IsDeployment && interpreter.evm.chainRules.IsVerkle {
			codeAddr := scope.Contract.CodeAddr
			statelessGas, wanted := interpreter.evm.Accesses.TouchCodeChunksRangeAndChargeGas(codeAddr[:], uint64(startMin), uint64(pushByteSize), uint64(len(scope.Contract.Code)), false, false, scope.Contract.Gas)
			scope.Contract.UseGas(statelessGas)
			if statelessGas < wanted {
				return nil, ErrOutOfGas
			}
