...
func (aw *AccessWitness) TouchAndChargeProofOfAbsence(addr []byte) uint64 {
	var gas uint64
	gas += aw.touchAddressAndChargeGas(addr, zeroTreeIndex, utils.VersionLeafKey, AccessListRead)
	gas += aw.touchAddressAndChargeGas(addr, zeroTreeIndex, utils.BalanceLeafKey, AccessListRead)
	gas += aw.touchAddressAndChargeGas(addr, zeroTreeIndex, utils.CodeSizeLeafKey, AccessListRead)
	gas += aw.touchAddressAndChargeGas(addr, zeroTreeIndex, utils.CodeKeccakLeafKey, AccessListRead)
	gas += aw.touchAddressAndChargeGas(addr, zeroTreeIndex, utils.NonceLeafKey, AccessListRead)
	return gas
}
...
func (aw *AccessWitness) TouchAndChargeContractCreateInit(addr []byte, createSendsValue bool) uint64 {
	...
	gas += aw.touchAddressAndChargeGas(addr, zeroTreeIndex, utils.CodeKeccakLeafKey, AccessListWrite)
	if createSendsValue {
...
func (aw *AccessWitness) TouchAndChargeContractCreateCompleted(addr []byte) uint64 {
	...
	gas += aw.touchAddressAndChargeGas(addr, zeroTreeIndex, utils.VersionLeafKey, AccessListWrite)
	gas += aw.touchAddressAndChargeGas(addr, zeroTreeIndex, utils.BalanceLeafKey, AccessListWrite)
	gas += aw.touchAddressAndChargeGas(addr, zeroTreeIndex, utils.CodeSizeLeafKey, AccessListWrite)
	gas += aw.touchAddressAndChargeGas(addr, zeroTreeIndex, utils.CodeKeccakLeafKey, AccessListWrite)
	gas += aw.touchAddressAndChargeGas(addr, zeroTreeIndex, utils.NonceLeafKey, AccessListWrite)
	return gas
}
...
func (aw *AccessWitness) TouchTxOriginAndComputeGas(originAddr []byte) uint64 {
	aw.touchAddress(originAddr, zeroTreeIndex, utils.VersionLeafKey, AccessListRead)
	aw.touchAddress(originAddr, zeroTreeIndex, utils.CodeSizeLeafKey, AccessListRead)
	aw.touchAddress(originAddr, zeroTreeIndex, utils.CodeKeccakLeafKey, AccessListRead)
	aw.touchAddress(originAddr, zeroTreeIndex, utils.NonceLeafKey, AccessListWrite)
	aw.touchAddress(originAddr, zeroTreeIndex, utils.BalanceLeafKey, AccessListWrite)
...
func (aw *AccessWitness) TouchTxExistingAndComputeGas(targetAddr []byte, sendsValue bool) uint64 {
	aw.touchAddress(targetAddr, zeroTreeIndex, utils.VersionLeafKey, AccessListRead)
	aw.touchAddress(targetAddr, zeroTreeIndex, utils.CodeSizeLeafKey, AccessListRead)
	aw.touchAddress(targetAddr, zeroTreeIndex, utils.CodeKeccakLeafKey, AccessListRead)
	aw.touchAddress(targetAddr, zeroTreeIndex, utils.NonceLeafKey, AccessListRead)
	if sendsValue {
		aw.touchAddress(targetAddr, zeroTreeIndex, utils.BalanceLeafKey, AccessListWrite)
	} else {
		aw.touchAddress(targetAddr, zeroTreeIndex, utils.BalanceLeafKey, AccessListRead)
	}
...
func (aw *AccessWitness) touchAddressAndChargeGas(addr []byte, treeIndex uint256.Int, subIndex byte, isWrite AccessListAccessMode) uint64 {
...
func (aw *AccessWitness) touchAddress(addr []byte, treeIndex uint256.Int, subIndex byte, isWrite AccessListAccessMode) (bool, bool, bool, bool, bool) {
...
func (aw *AccessWitness) AddAddress(addr common.Address, isWrite AccessListAccessMode) uint64 {
	...
	gas += aw.touchAddressAndChargeGas(addr[:], zeroTreeIndex, utils.CodeKeccakLeafKey, isWrite)
